...
// Descarga Plantilla en html
// helpers para cargar/renderizar plantillas
...
// {{&var}} sin escape (para logoData); {{var}} escapado
function renderTemplate(tpl, vars) {
  let html = tpl.replace(/\{\{\&\s*([\w.]+)\s*\}\}/g, (_, k) => vars[k] != null ? String(vars[k]) : '');
  html = html.replace(/\{\{\s*([\w.]+)\s*\}\}/g, (_, k) => escapeHTML(vars[k] != null ? String(vars[k]) : ''));
  return html;
}

export const descargarPlantillaEditable = async (req, res) => {
  const id = Number.parseInt(req.params.id, 10);
  const tipo = String(req.params.tipo || '').toLowerCase(); // 'ccm' | 'conformidad'
  if (!id || !['ccm', 'conformidad'].includes(tipo)) {
    return res.status(400).json({ error: 'Parámetros inválidos' });
  }

  // Recupero la OT y armo las mismas variables que ya usás:
  const orden = await prisma.ordenTrabajo.findUnique({
    where: { id },
    include: {
      empleadosAsignados: { include: { empleado: true } },
  ...
  try {
    ...
    if (fs.existsSync(logoPath)) {
      ...
      logoData = `<img src="data:image/jpeg;base64,${buf.toString('base64')}" alt="logo" />`;
    }
  } catch {}
  ...
  const motorSnap = componentesSnap.find(c => (c?.tipo || '').toLowerCase() === 'motor') || null;
  const avionTSN = avSnap.TSN ?? '';
  const cert = (orden.empleadosAsignados || []).find(e => (e?.rol || '').toUpperCase() === 'CERTIFICADOR')?.empleado
            || (orden.registrosTrabajo || []).find(r => (r?.rol || '').toUpperCase() === 'CERTIFICADOR')?.empleado
            || (orden.empleadosAsignados || []).find(e => e?.empleado?.esCertificador)?.empleado
            || null;

  const licNumero = cert?.numeroLicencia ? `MMA. ${cert.numeroLicencia}` : '';
  const licVenc = cert?.vencimientoLicencia ? fmtUY(cert.vencimientoLicencia) : '';
  const licFull = licNumero ? `${licNumero}${licVenc ? ` - Venc.: ${licVenc}` : ''}` : (licVenc ? `Venc.: ${licVenc}` : '');

  // Vars pre-rellenas (fecha/lugar/aeronave/motor quedan vacías para que el usuario las edite)
  const vars = {
    fechaEmision: fmtUY(new Date()),
    empresaTitulo: 'CELCOL AVIATION',
    empresaLinea1: 'Camino Melilla Aeropuerto Ángel Adami',
    empresaLinea2: 'Sector CAMES – Hangar Nº 2 · OMA IR-158',

    matricula: avSnap.matricula ?? orden.avion?.matricula ?? '',
    marca:     avSnap.marca     ?? orden.avion?.marca     ?? '',
    modelo:    avSnap.modelo    ?? orden.avion?.modelo    ?? '',
    serial:    avSnap.numeroSerie ?? orden.avion?.numeroSerie ?? '',

    lugar:  '',
    horasTT: String(avionTSN || ''),
    ot:     String(orden.numero ?? orden.id),
    ...
    certificadorNombre: [cert?.nombre, cert?.apellido].filter(Boolean).join(' '),
    certificadorLicString: licFull,

    motorMarca:  motorSnap?.marca       ?? '',
    motorModelo: motorSnap?.modelo      ?? '',
    motorSerial: motorSnap?.numeroSerie ?? '',
    motorTSO:    motorSnap?.TSO ?? motorSnap?.tso ?? '',
    motorTBOHoras: motorSnap?.TBOHoras ?? motorSnap?.tboHoras ?? motorSnap?.TBO ?? motorSnap?.tbo ?? '',
  ...
  const filename = tipo === 'ccm'
    ? `Plantilla-CCM-OT-${orden.numero ?? orden.id}.html`
    : `Plantilla-Conformidad-OT-${orden.numero ?? orden.id}.html`;

  const tplName = tipo === 'ccm' ? 'ccm.html' : 'conformidad.html';
  const tplContent = safeReadTemplate(tplName);
  if (!tplContent) return res.status(404).json({ error: `Falta templates/${tplName}` });

  const html = renderTemplate(tplContent, vars);
