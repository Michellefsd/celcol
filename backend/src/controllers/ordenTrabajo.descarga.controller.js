...
export const descargarOrdenPDF = async (req, res) => {
  ...
  const pintarPropietario = (doc, p) => {
    ...
    const tipo = (p.tipoPropietario || '').toUpperCase();
    if (tipo === 'INSTITUCION') {
      doc.text(`Razón Social / Empresa: ${p.nombreEmpresa || '-'}`);
    } else {
  ...
  try {
    ...
    // ===== HASTA AQUÍ: HEADERS =====
    // A partir de aquí irían las cajas 1–5, la tabla 6–10, el bloque A/B y la caja 13.
    // (Los dibujo en el siguiente bloque cuando me digas).
    
    // No cerramos el PDF acá; el resto del contenido continúa en el siguiente segmento.
    // Si querés probar el header solo, podés temporalmente hacer:
    // doc.end(); return;
  } catch (error) {
    ...
    // ===== CONTENIDO CA-29 DESPUÉS DEL HEADER =====

    // --- Helpers locales para grilla/tabla ---
    const cell = (doc, x, y, w, h, text = '', opts = {}) => {
      const { label, labelSize = 8, textSize = 10, boldLabel = false } = opts;
      doc.save();
      doc.rect(x, y, w, h).stroke();
      if (label) {
        doc.fontSize(labelSize).font(boldLabel ? 'Helvetica-Bold' : 'Helvetica')
           .text(label, x + 4, y + 2, { width: w - 8, height: 10 });
      }
      if (text) {
        doc.fontSize(textSize).font('Helvetica')
           .text(text, x + 4, y + (label ? 14 : 4), { width: w - 8, height: h - (label ? 18 : 8) });
      }
      doc.restore();
    };

    const t = (doc, text, x, y, size = 10, bold = false, align = 'left', width = null) => {
      doc.save();
    ...
    const tablaItems = (doc, x, y, widths, rowHeight, filas) => {
      // Encabezado
      const headers = ['#', '6 Reporte', '7 Acción Tomada', '8 Técnico', '9 Certific.', '10 H.H'];
      let cx = x;
      for (let i = 0; i < widths.length; i++) {
        cell(doc, cx, y, widths[i], rowHeight, headers[i], { boldLabel: true, labelSize: 10 });
        cx += widths[i];
      }
      // Filas (4)
      let yy = y + rowHeight;
      for (let r = 0; r < 4; r++) {
        ...
        cx = x;
        for (let i = 0; i < widths.length; i++) {
          cell(doc, cx, yy, widths[i], rowHeight, null, {});
          t(doc, data[i], cx + 4, yy + 6, 10, false, 'left', widths[i] - 8);
          cx += widths[i];
        }
        yy += rowHeight;
      }
    ...
    const tablaAB = (doc, x, y, widths, rowHeight, A, B) => {
      ...
      const drawRow = (tag, row, yy) => {
        ...
        let cxx = x;
        for (let i = 0; i < widths.length; i++) {
          cell(doc, cxx, yy, widths[i], rowHeight, null, {});
          t(doc, data[i], cxx + 4, yy + 6, 10, false, 'left', widths[i] - 8);
          cxx += widths[i];
        }
      };
      let yy = y + rowHeight;
    ...
    // ===== Cajas 1–3 (en una fila) =====
    // 1 Matrícula | 2 Fecha solicitud | 3 OT Nro.
    const X = 36, WIDTH = 523;
    const col1 = 180, col2 = 160, col3 = WIDTH - col1 - col2; // suman 523
    const rowH = 42;

    const matricula = (normalizeSnap(orden.datosAvionSnapshot)?.matricula) ||
    ...
    cell(doc, X + col1 + col2, curY, col3, rowH, String(orden.numero ?? orden.id), { label: '3 OT Nro.', boldLabel: true });
    curY += rowH + 6;

    // ===== Caja 4 (Solicitud) + Caja 5 (Firma o email) =====
    const solicitudH = 78;
    const firmaW = 200;
    const solicitudW = WIDTH - firmaW - 6; // separador
    const solicitudTexto = orden.solicitud || orden.descripcionTrabajo || orden.descripcion || '';
    const firmaTexto = orden.solicitadoPor || orden.solicitanteEmail || '';
    ...
    // widths suman 523 exacto: [28, 240, 150, 40, 40, 25]
    const tX = X, tW = [28, 240, 150, 40, 40, 25], tRowH = 42;
    ...
    const tecnicos = (Array.isArray(personalSnap) ? personalSnap : (orden.empleadosAsignados || []))
      .filter(e => (e?.rol || '').toUpperCase() === 'TECNICO')
      .map(toNombre).filter(Boolean);
    const certificadores = (Array.isArray(personalSnap) ? personalSnap : (orden.empleadosAsignados || []))
    ...
    // Completar filas vacías hasta 4
    while (filas.length < 4) filas.push({ numero: filas.length + 1 });

    curY = tablaItems(doc, tX, curY, tW, tRowH, filas) + 8;

    // ===== Bloque A/B (11–12) =====
    // Podés mapear desde un snapshot si lo tenés, por ahora vacío para completar manualmente
    const discA = normalizeSnap(orden.discrepanciasSnapshot)?.A || {};
    const discB = normalizeSnap(orden.discrepanciasSnapshot)?.B || {};

    curY = tablaAB(doc, tX, curY, tW, tRowH, discA, discB) + 10;

    // ===== Caja 13 (Fecha de cierre) =====
    const fechaCierreTexto = estado === 'CERRADA'
      ? fmtUY(orden.fechaCierre)
      : fmtUY(orden.fechaCancelacion); // si está cancelada, usamos fecha cancelación
    cell(doc, X, curY, 220, 36, fechaCierreTexto || '', { label: '13 Fecha de cierre', boldLabel: true });
    curY += 50;

    // ===== Pie de página =====
    const footY = 800; // forzado cerca del pie A4
