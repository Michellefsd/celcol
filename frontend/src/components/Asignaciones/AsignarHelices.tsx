...
export default function AgregarHeliceModal({ avionId, onClose, onSaved }: {
  avionId: number;
  onClose: () => void;
  onSaved?: () => void;
}) {
  ...
  const handleSubmit = async () => {
    ...
    await fetch('http://localhost:3001/helices', {
      method: 'POST',
  ...
  return (
    ...
    <div className="bg-white p-6 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto space-y-4">
        <h2 className="text-xl font-semibold">Agregar Hélice</h2>
