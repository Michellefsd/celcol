...
export default function EditarMotorModal({
  motor,
  onClose,
  onSaved
}: {
  ...
  useEffect(() => {
    if (motor) {
      setFormData({
        ...
        TSN: motor.TSN?.toString() || '',
        TSO: motor.TSO?.toString() || '',
        TBOFecha: motor.TBOFecha ? motor.TBOFecha.slice(0, 10) : '',
        TBOHoras: motor.TBOHoras?.toString() || ''
      });
    }
  }, [motor]);
  ...
  const handleSubmit = async () => {
    ...
    await fetch(`http://localhost:3001/motores/${motor.id}`, {
      method: 'PUT',
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-xl shadow-xl max-w-lg w-full max-h-[90vh] flex flex-col">
        <div className="p-4 border-b sticky top-0 bg-white z-10">
          <h2 className="text-xl font-semibold">Editar Motor</h2>
        </div>
